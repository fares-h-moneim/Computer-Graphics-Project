...
namespace our
{
    ...
    struct PipelineState
    {
        ...
        void setup() const
        {
            ...
            // Face Culling: Rendering only the faces that are visible to the camera to save fragment shader calls
            if (faceCulling.enabled)
            {
                glEnable(GL_CULL_FACE);             // To enable face culling
                glCullFace(faceCulling.culledFace); // To specify which face to cull
                glFrontFace(faceCulling.frontFace); // To specify which face is the front face
            }
            else
            ...
            if (depthTesting.enabled)
            {
                ...
                glDepthFunc(depthTesting.function); // To specify the depth function
            }
            else
            ...
            if (blending.enabled)
            {
                ...
                glBlendEquation(blending.equation);                                                                                   // To specify the blending equation
                glBlendFunc(blending.sourceFactor, blending.destinationFactor);                                                       // To specify the blending factors
                glBlendColor(blending.constantColor.r, blending.constantColor.g, blending.constantColor.b, blending.constantColor.a); // To specify the blending color
            }
            else
